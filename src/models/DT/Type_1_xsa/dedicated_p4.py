...
def separate_metadata(fname, config, xsa_architecture=False):
    # this should not be called for xsa_architecture
    if xsa_architecture:
        return
...
def separate_variables(fname, config, min_code_width=10):
    with open(fname, 'a') as processing:
        for i in range(0, config['num_features']):
            processing.write(
                    "    bit<" + str(max(int(config['code_width'][i]), min_code_width)) + "> code_f" + str(i) + ";\n")
...
def separate_tables(fname, config, xsa_architecture=False):
    # vitis P4 compiler requires minimum key width of 10 bits
    if xsa_architecture:
        min_code_width = 10
    else:
        min_code_width = 0
    with open(fname, 'a') as ingress:
        for i in range(0, config['num_features']):
            ingress.write("    action extract_feature" + str(i) + "(out bit<" + str(
                max(int(np.array(config['code_width'])[i]), min_code_width)) + "> meta_code, bit<" + str(
                max(int(np.array(config['code_width'])[i]), min_code_width)) + "> tree){\n" \
                                                                  "        meta_code = tree;\n" \
                                                                  "    }\n\n")
        ...
        for j in range(0, config['num_features']):
            if xsa_architecture:
                ingress.write("            code_f" + str(j) + ":exact;\n") 
            else:
                ingress.write(
                    "           meta.code_f" + str(j) + "[" + str(int(count_code[j] + config['code_width'][j] - 1)) + ":" + str(int(count_code[j])) + "]:exact;\n                ") # not sure why indexing is necessary here
            count_code[j] += config['code_width'][j] # why is this necessary??
        ingress.write("        }\n")
...
def create_tables_Commend_esnet(fname, config):
    # commands to load table entries using esnet-smartnic-fw
    num_features = config['data config']['number of features']
    num_classes = config['model config']['number of classes']
    Exact_Table = json.load(open('Tables/Exact_Table.json', 'r'))
    with open(fname, 'w') as file:
