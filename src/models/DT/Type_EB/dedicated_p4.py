...
def create_tables(Planter_config):
    ...
    for idx in Ternary_Table['code to vote']:
        ...
        for f in range(num_features):
            ...
            Entry["match"][key] = int(Ternary_Table['code to vote'][idx]['f'+str(f)+' code'])
        Entry["action_name"] = "SwitchIngress.read_lable"
        Entry["action_params"] = {}
        Entry["action_params"]["label"] = int(Ternary_Table['code to vote'][idx]['leaf'])
        Table_entries += [Entry]
    for f in range(num_features):
    ...
    with open(fname, 'a') as tload:
        ...
        for i in range(0, config['num_features']):
            ...
            tload.write("    " + (config['debug_load_table'] * "# ") + \
                        ...
                        "(table['feature " + str(i) + "'][key][1], table['feature " + str(i) + "'][key][0], int(key), int(codes,2))\n")
