...
def create_tables(Planter_config):
    # change this in topology.json to activate: "s1": {"runtime_json": "s1-runtime.json"}
    Table_entries = []
    ...
    for idx in LPM_Table['code to vote']:
        ...
        for f in range(num_features):
            ...
            Entry["match"][key] = int(LPM_Table['code to vote'][idx]['f'+str(f)+' code'])
        Entry["action_name"] = "SwitchIngress.read_lable"
        Entry["action_params"] = {}
        Entry["action_params"]["label"] = int(LPM_Table['code to vote'][idx]['leaf'])
        Table_entries += [Entry]
    for f in range(num_features):
...
def create_tables_Commend(fname, config):
    ...
    with open(fname, 'w') as file:
        ...
        for idx in LPM_Table['code to vote']:
            ...
            for f in range(num_features):
                file.write(str(LPM_Table['code to vote'][idx]['f' + str(f) + ' code'])+" ")
            file.write("=> "+str(LPM_Table['code to vote'][idx]['leaf'])+"\n")
