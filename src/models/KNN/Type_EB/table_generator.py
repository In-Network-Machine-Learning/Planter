...
def clustream(table, idx, code, lookup, num_features, num_classes, num_depth, depth_num, center, width, border_max, border_min, knn_clf, division, is_return):
    ...
    # print('0. center', center, 'width', width,'depth',depth_num,'division', division)
    not_finish = True
    if depth_num ==0:
        not_finish = True
        ...
        _, cla = check_if_not_finish(copy.deepcopy(center), copy.deepcopy(width), division, num_features,
                                              num_depth, num_classes, knn_clf)
    # print('1. ', not_finish, cla)

    if not_finish:
        # print('go depper:', depth_num)
...
def test_tables(sklearn_test_y, test_X, test_y):
    ...
    for i in range(np.shape(test_X.values)[0]):
        ...
        for count in keys:
            ...
            # if input_feature_value[f] & Ternary_Table[count][0] == Ternary_Table[count][0] & Ternary_Table[count][1]:
