...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    question = 'Threshold value for binarization?'
    default = 10
    Planter_config = take_CLI_input(Planter_config, 'model config', 'binarize threshold', question, default,
                                    manually_input=False, numeric=True)
    threshold = Planter_config['model config']['binarize threshold']
    # =================== set max leaf node in config ===================
    question = 'Number of bits for each action data?'
    default = 16
    Planter_config = take_CLI_input(Planter_config, 'model config', 'number of bits', question, default,
                                    manually_input=False, numeric=True)
    ...
    # fit
    # clf = GaussianNB()
    clf = BernoulliNB(alpha=0, binarize= threshold, class_prior=None, fit_prior=True)
    clf.fit(train_X, train_y)
    sklearn_y_predict = clf.predict(test_X)
    result = classification_report(test_y, sklearn_y_predict, digits=4)
    print('\n', result)
    if num_classes==2:
        tn, fp, fn, tp = metrics.confusion_matrix(test_y, sklearn_y_predict).ravel()
        precision, sensitivity, specificity = tp/(tp+fp), tp/(tp+fn), tn/(tn+fp)
        print('\nPrecision:', precision)
        print('\nSensitivity:', sensitivity)
        print('\nSpecificity:', specificity)
        print('\nAccuracy:', (tp+tn)/(tn+fp+fn+tp))
        print('\nF1-Score:', 2*(precision*sensitivity)/(precision+sensitivity))
        print('\nMCC:', metrics.matthews_corrcoef(test_y, sklearn_y_predict))
        print('\nBCR:', metrics.balanced_accuracy_score(test_y, sklearn_y_predict))
    ...
    for c in range(num_classes):
        ...
        for f in range(num_features):
            ...
            model_parmeters['c' + str(c)]["f"+str(f)]['log_prob'] = clf.feature_log_prob_[c,f]
        ...
        for inputs in range(0,feature_max[f]+1):
            ...
            for c in range(num_classes):
                if inputs > threshold:
                    ...
                    x = 0
                partial_neg_prob = np.log(1- np.exp(model_parmeters['c' + str(c)]["f"+str(f)]['log_prob']))
                partial_jll = x * (model_parmeters['c' + str(c)]["f"+str(f)]['log_prob'] - partial_neg_prob)

                value = copy.deepcopy(partial_jll + partial_neg_prob)
    ...
    for c in range(num_classes):
        value = clf.class_log_prior_[c]
        Bayes_separate_table["class prob"]['class '+str(c)]= value
        if value > value_info["max"]:
            value_info["max"] = value
        if value < value_info["min"]:
            value_info["min"] = value
