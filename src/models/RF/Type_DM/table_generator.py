            ...
            if tree_.feature[node] != _tree.TREE_UNDEFINED:
                ...
                with open(fname, 'a') as command:
                    command.write("table_add SwitchIngress.level_"+str(i_tree)+ "_"+str(depth)+ " SwitchIngress.CheckFeature"
                                  +str(i_tree)+"_"+ str(name)+ " "+ str(previous_id) + " " + str(prevfeature) + " "+
                                  str(result) +  " => " + str(current_id) + " " + str(int(float(threshold)))+"\n")
                global first_entry
                global entry_info
                global Exact_Table

                Exact_Table['node table'][Exact_Table['node table counter']] = ["CheckFeature"+str(i_tree)+"_"+ str(name),
                                                                                "level_"+str(i_tree)+ "_"+str(depth),
                                                                                str(previous_id), str(prevfeature),
...
def run_model(train_X, train_y, test_X, test_y, used_features):
    ...
    Planter_config['model config']['number of trees'] = int(input('- Number of trees? (default = 3) ') or '3')
    Planter_config['model config']['number of depth'] = int(input('- Number of depth? (default = 2) ') or '2')
    Planter_config['model config']['max number of leaf nodes'] = int(input('- Number of leaf nodes? (default = 10000) ') or '10000')
