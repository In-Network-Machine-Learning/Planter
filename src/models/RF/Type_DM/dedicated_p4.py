...
def separate_metadata(fname, config):
    with open(fname, 'a') as headers:
        ...
        for t in range(0, config['num_trees']):
            headers.write("    bit<16> node_id"+str(t+1)+";\n"
...
def separate_logics(fname, config):
    with open(fname, 'a') as ingress:
        ...
        ingress.write("\n")
        for t in range(0, config['num_trees']):
            ingress.write("        meta.node_id"+str(t+1)+" = "+str(config['first_entry'][t][0])+";\n"
    ...
    with open(fname, 'a') as ingress:
        for t in range(0, config['num_trees']):
            for f in range(0, config['num_features']):
                ingress.write("    action CheckFeature"+str(t+1)+"_"+str(f)+"(bit<16> node_id, bit<32> threshold) {\n"
        ...
        for t in range(0, config['num_trees']):
            for d in range(0, config['num_depth']+1):
                ingress.write("    @pragma stage "+str(d+1)+"\n")
                ingress.write("    table level_"+str(t+1)+"_"+str(d+1)+"{\n"
                              ...
                              "            NoAction;\n")
                for f in range(0, config['num_features']):
                    ingress.write("            CheckFeature"+str(t+1)+"_"+str(f)+";\n")
