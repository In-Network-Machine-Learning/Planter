...
def separate_tables(fname, config, xsa_architecture=False):
    ...
    with open(fname, 'a') as ingress:
        ...
        for i in range(0, config['num_features']):
            ...
            if xsa_architecture:
                ingress.write("        key = { hdr.Planter.feature" + str(i) + ":lpm; }\n")
            else:
                ingress.write("        key = { meta.feature" + str(i) + ":lpm; }\n")
        ...
        for i in range(config['num_trees']):
            ...
            if xsa_architecture:
                ingress.write("        tree_" + str(i) + "_vote = " + str(config['default_vote']) + ";\n")
            else:
                ingress.write("        meta.tree_" + str(i) + "_vote = " + str(config['default_vote']) + ";\n")
            ingress.write("    }\n\n")
        ...
        for i in range(config['num_trees']):
            ...
            for j in range(0, config['num_features']):
                if xsa_architecture:
                    ...
                    ingress.write(
                        "(bit<" + str(max(key_len, min_code_width)) + ">) code_f" + str(j) + "[" + str(int(count_code[j] + config['code_width'][i][j] - 1)) + ":" + str(int(count_code[j])) + "]:exact @name(\"lookup_leaf_id" + str(i) + "\");\n                ")
                else:
                    ingress.write(
        ...
        if xsa_architecture:
            ingress.write("        hdr.Planter.result = " + str(config['default label']) + ";\n")
        else:
            ingress.write("        meta.result = " + str(config['default label']) + ";\n")
        ingress.write("    }\n\n")
        ...
        for f in range(num_features):
            for idx in LPM_Table['feature ' + str(f)]:
                ...
                file.write("table_add SwitchIngress.lookup_feature" + str(f) + " extract_feature" + str(f) +
                           ...
                           str(((1<<8)-1)&(key))+ "/" + str(32 - int(math.log(2 ** config['p4 config']["width of feature"][f] - mask, 2))) +
                           " => " + str(label) + " \n")
...
def create_tables_Commend_esnet(fname, config):
    ...
    with open(fname, 'w') as file:
        ...
        for f in range(num_features):
            for idx in LPM_Table['feature ' + str(f)]:
                # priority = int(idx)
                priority = len(LPM_Table['feature ' + str(f)]) - int(idx)
                key = LPM_Table['feature ' + str(f)][idx][1]
                mask = LPM_Table['feature ' + str(f)][idx][0]
                codes = ''
                for t in range(num_trees):
                ...
                file.write("table_add lookup_feature" + str(f) + " extract_feature" + str(f) +
                           " " + hex(key) + "/" + str(32 - int(math.log(2 ** config['p4 config']["width of feature"][f] - mask, 2))) +
                           " => " + hex(label) + " " + str(priority) + "\n")
