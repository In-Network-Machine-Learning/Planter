...
def generate_test_file(config_file):
    ...
    ##################################################
    # predict time
    commands_file = Planter_config['directory config']['work'] + '/src/targets/bmv2/software/model_test/test_environment/s1-commands.txt'
...
def main(sklearn_test_y, test_X, test_y, processes, if_using_subprocess):
    ...
    run_switch_model_command = work_root + '/src/scripts/run_switch_model.sh'
    task1_run_model = Process(target=run_command, args=(str(run_switch_model_command), str(model_test_root),))
    task1_run_model.daemon = True
    task1_run_model.start()
    processes.append(task1_run_model)
    ...
    # =================== run the switch model ===================
    load_table_command = work_root + '/src/scripts/simple_switch_CLI_load_table.sh'
    # print(run_command, str(load_table_command), str(model_test_root))
    task2_load_table = Process(target=run_command, args=(str(load_table_command), str(model_test_root),))
    ...
    test_switch_model_command = work_root + '/src/scripts/test_switch_model.sh'
    task3_test_model = Process(target=run_command, args=(str(test_switch_model_command), str(model_test_root),))
    task3_test_model.daemon = True
    task3_test_model.start()
    processes.append(task3_test_model)
