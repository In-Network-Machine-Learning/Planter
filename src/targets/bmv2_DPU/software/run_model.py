...
def add_test_model(fname, config):
    ...
    test_file = config['directory config']['work'] + '/src/test/' + test_file_name + '.py'
    work_root, model_test_root, file_name, test_file_name = file_names(config)
    password = config['test config']['sudo password']
...
def main(if_using_subprocess):
    ...
    # =================== load table command for the generated model ===================
    test_model_command = Planter_config['directory config']['work'] + '/src/scripts/test_switch_model.sh'
