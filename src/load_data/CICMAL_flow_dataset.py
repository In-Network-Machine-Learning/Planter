...
def load_data(num_features, data_dir):
    ...
    data = data[(data[' Flow Duration']>0) & (data[' Flow IAT Min']>0) & (data[' Min Packet Length']>0) & (data[' Flow IAT Mean']>0) & (data[' Flow IAT Max']>0)]
    used_features = [' Flow Duration', ' Flow IAT Mean', ' Flow IAT Max', ' Flow IAT Min', ' Min Packet Length', ' Source Port', ' Destination Port', ' SYN Flag Count', ' RST Flag Count', ' ACK Flag Count'][:num_features]
